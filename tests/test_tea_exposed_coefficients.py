...
def test_other_g_constructor_coefficients_are_not_dumped():
    ...
    with pytest.raises(tea._ScenarioInputError) as caught:
        tea._scenario_config(_public_from_record(record, contingency=0.80))
    assert caught.value.error_code == "unknown_process_field"
    assert "contingency" in list(caught.value.details.get("extra_keys") or [])
    with pytest.raises(tea._ScenarioInputError) as caught:
    ...
    assert "contingency" not in tea._COEFFICIENT_DEFAULTS
