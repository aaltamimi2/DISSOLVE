"""v3 §8 commits 3–4: contaminant_mode binds the planner; wash/STRAP embed.

Accept test 2 is DEHP / toluene / {PET,EVOH} fail vs {EVOH} pass, both rows rt.
Do not weaken it. No BioSTEAM. Regex / CAS alias table is not this commit.
"""
...
def test_refusals_stay_constructed():
    ...
    family = _data(separation.plan_multistage_separation(
        ["LDPE", "PP"], contaminants="BFR", contaminant_mode="strap",
    ))
...
def test_accept_test_2_on_evaluator():
...
def test_leaching_enumerates_positions_and_names_the_objective():
    ...
    assert "passing_count" in winner
    assert "contaminant_logd_min" in winner
    assert "index" in winner
    washes = [
        item for item in (payload.get("steps") or [])
