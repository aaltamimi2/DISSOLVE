...
def _validate_contaminant_plan(
    calls: Sequence[dict[str, Any]], query: str, context: dict[str, Any],
) -> tuple[str, ...]:
    """Preserve removal mode, inherited identities, and exclusive bounds."""
    if not calls:
        return ()
    call = calls[0]
    arguments = call.get("args") or {}
    if not isinstance(arguments, dict):
        return ("Contaminant tool arguments must be one JSON object.",)

    declared_tool = _CONTAMINANT_REQUEST_TO_TOOL.get(str(context.get("declared_request_kind") or ""))
    fallback_tool = str(call.get("name") or "")
    expected_tool = declared_tool or fallback_tool
    # request_kind is the structural cross-check for production handoffs.
    # The specialist's typed tool selection is the compatibility fallback when
    # an older parent omits that optional declaration. Keep that path
    # planner-first:
    # do not reconstruct the process choice from user prose.

    violations: list[str] = []
    if call.get("name") != expected_tool:
        violations.append(f"Use {expected_tool} for the requested removal mode.")

    explicit_thresholds = _query_threshold_overrides(query)
    prior = context.get("last_contaminant") or {}
    prior_thresholds = dict(prior.get("active_thresholds_wt_pct") or {})
    prior_sources = dict(prior.get("threshold_sources") or {})
    inherited_thresholds = {
        key: value for key, value in prior_thresholds.items()
        if prior_sources.get(key) == "user"
    }
    expected_thresholds = {**inherited_thresholds, **explicit_thresholds}
    applicable = set(expected_thresholds)
    threshold_keys = set(default_thresholds(
        include_precipitation=(
            expected_tool != "screen_contaminant_leaching"
        ),
    ))
    applicable &= threshold_keys
    if applicable:
        for key in sorted(applicable):
            expected = _finite(expected_thresholds[key])
            supplied = _finite(arguments.get(key))
            if expected is not None and supplied != expected:
                violations.append(
                    f"Preserve active {key}={expected:g}; these screening bounds "
                    "are user-controlled and must not revert to defaults."
                )
    unrequested = sorted(
        key for key in threshold_keys - applicable
        if arguments.get(key) is not None
    )
    if unrequested:
        violations.append(
            "Remove unrequested contaminant-threshold arguments: "
            + ", ".join(unrequested)
            + ". Passing documented defaults explicitly relabels them as "
            "user-requested bounds. Accepted plan: omit these keys so they "
            "remain default-proxy values; pass only thresholds stated by the "
            "user now or preserved from an earlier user override."
        )

    declared = context.get("declared_deliverable")
    typed_bound = declared if isinstance(declared, dict) else {}
    maximum, strict = _finite(typed_bound.get("temperature_max_c")), bool(typed_bound.get("strict_maximum"))
    if not typed_bound: maximum, strict = _query_bound(query, context)
    if maximum is not None and (
        _finite(arguments.get("max_temperature_c")) != maximum
        or bool(arguments.get("strict_maximum")) != strict
    ):
        violations.append(f"Preserve max_temperature_c={maximum:g} and strict_maximum={str(strict).lower()}.")

    named_polymers = {
        polymer
        for label, _ in thermo.find_polymer_mentions(query)
        for polymer in thermo.expand_polymer_identity(label)
    }
    # The typed-bound declaration outranks inherited state: a target the
    # planner resolved through the validated manifest is the referent the
    # specialist must screen, whether or not an earlier screen stored a
    # different one.
    declared_targets = [
        resolved
        for item in typed_bound.get("target_polymers") or []
        if (resolved := thermo.resolve_polymer(str(item)))
    ]
    expected_target = (
        declared_targets[0]
        if len(declared_targets) == 1
        else str(prior.get("target_polymer") or "")
    )
    if expected_target:
        supplied_target = thermo.resolve_polymer(str(arguments.get("target_polymer") or ""))
        if supplied_target != expected_target:
            violations.append(f"Preserve declared target_polymer={expected_target}.")
    inherits_identity = bool(prior and (not named_polymers or expected_target in named_polymers))
    if inherits_identity:
        prior_contaminants = [str(item) for item in prior.get("requested_contaminants") or []]
        current_family = mentions_contaminant_family(query)
        if prior_contaminants and not current_family:
            supplied = {_key(item) for item in _items(arguments.get("contaminants"))}
            expected = {_key(item) for item in prior_contaminants}
            if supplied != expected:
                violations.append(
                    "Preserve inherited contaminants: " + ", ".join(prior_contaminants) + "."
                )
        prior_others = [str(item) for item in prior.get("other_polymers") or []]
        if prior_others:
            supplied_others = {
                thermo.resolve_polymer(item) for item in _items(arguments.get("other_polymers"))
            } - {None}
            if supplied_others != set(prior_others):
                violations.append(
                    "Preserve inherited other_polymers: " + ", ".join(prior_others) + "."
                )
...
CONTAMINANT_PROMPT = """You are DISSOLVE's contaminant-removal analyst. Obey the
typed declared_request_kind from the parent: contaminant_leaching requires
screen_contaminant_leaching, contaminant_strap_removal requires
screen_contaminant_strap_removal, and contaminant_mode_comparison requires
compare_contaminant_removal_modes. Call exactly one tool and preserve the target
polymer, PFAS/phthalate family or compound names, other polymers, candidate
solvents, and maximum temperature from the request or typed state. For a request
below, under, or less than a ceiling, pass that ceiling unchanged and set
strict_maximum=true; for the same-range follow-up, inherit both the ceiling and
its strict/inclusive meaning. Candidate scope is structural: omit solvents to
inherit prior typed candidates, or pass solvents as an explicit override; pass
an empty list to request the tool's full admitted scope. Preserve an explicit
swelling window, minimum dissolution threshold, and swing precipitation proxy as
the corresponding typed tool arguments. Omit those arguments when the user did
not choose them so the documented defaults remain identifiable. If a follow-up
changes a threshold, pass the new value; if it changes only the mode after a
user-defined screen, preserve the active user-defined thresholds from typed
state. The tool enforces 0 < swelling minimum < swelling maximum <= dissolution
minimum <= 100 wt%, and a positive swing precipitation proxy below the
dissolution minimum; never clamp or repair invalid bounds yourself. Never
evaluate or report an excluded endpoint. For a request that names unsupported
contaminants, still call the comparison tool so its structured scope error
preserves the target polymer, other polymers, exact unsupported names, and
supported families; never substitute a guessed contaminant. For that unsupported
case, call compare_contaminant_removal_modes only; do not call the leaching-only
or STRAP-only screen. Treat a recognized polymer fragment as other_polymers, not
as a contaminant name. Never turn workbook miscibility/logD, modeled swelling,
or precipitation thresholds into validated partition coefficients, removal
efficiency, recovery, kinetics, or purity. Return no prose; the parent model
explains the compact result."""
