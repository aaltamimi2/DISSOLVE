...
def _extract_prompt(pages: list[int]) -> str:
    ...
    return (
        ...
        "paper. Files: "
        + listed
        + ". Extract every visible table as JSON only, no prose:\n"
        '{"tables":[{"locus":"Table N or Fig. N or null","page":<int>,'
        '"rows":[{"polymer":"","solvent":"","temperature":"","value":"",'
        '"cells":["..."]}]}]}\n'
        "Use empty strings when a field is not visible. Do not invent values."
    )
...
def _needles_from_row(row: Mapping[str, Any]) -> dict[str, str]:
...
def bind_table_cells(
    *,
    ...
    for table in (channel_c_result.get("reading") or {}).get("tables") or []:
...
def c_and_cprime_agree(row_c: Mapping[str, Any], row_cp: Mapping[str, Any]) -> dict[str, str] | None:
    ...
    if cells_c and cells_c == cells_p:
        return needles_c if len(needles_c) == 4 else {}
    return None
    ...
    *,
    ...
    for table in (channel_c_result.get("reading") or {}).get("tables") or []:
        for row in table.get("rows") or []:
            ...
            rows_c.append(item)
    rows_p = []
    for table in (channel_c_prime_result.get("reading") or {}).get("tables") or []:
        for row in table.get("rows") or []:
            item = dict(row)
            item.setdefault("locus", table.get("locus"))
            item.setdefault("page", table.get("page"))
    ...
    for row_c in rows_c:
        ...
        agreed.append({
            ...
            "locus": locus,
            "page": row_c.get("page"),
            "needles": needles if len(needles) == 4 else {},
            "channels_used": ["C", "C_prime"],
            "read_by": {"C": model_c, "C_prime": model_p},
            "readings": {"A": None, "B": None, "C": row_c, "C_prime": row_p},
            "confidence": "unanimous" if len(needles) == 4 else "majority",
            "status": "gold_unsealed" if len(needles) == 4 else "awaiting_needles",
            "sealed": False,
        })
...
def apply_vision(
    *,
    ...
    figure_facts: list[dict[str, Any]] = []
    figure_disputes: list[dict[str, Any]] = []
    if fig_pages:
