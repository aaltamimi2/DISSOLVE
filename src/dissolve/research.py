...
# --- literature_ingest: Autonomous, typed literature extraction into the durable DISSOLVE graph.
...
def _library_slug(value: str) -> str:
...
def _parse(acquisition: Mapping[str, Any]) -> dict[str, Any]:
    ...
    """
    source = Path(str(_source_artifact(acquisition)["packed_path"]))
...
def ingest_literature_graph_data(
    ...
    library_id = _library_slug(library_id)
    if not paths:
    ...
    for parsed in parsed_documents:
        ...
        for index, candidate in enumerate(candidates):
            try:
                row = _normalize_candidate(candidate, parsed, library_id, index)
                validation = validate_literature_record(row, parsed_documents=[parsed], library_id=library_id)
                ...
                code = error.code if isinstance(error, LiteratureContractError) else type(error).__name__
    ...
    validation = validate_extraction_batch(records, parsed_documents=parsed_documents, library_id=library_id)
    ...
    batch["idempotency_key"] = derive_graph_idempotency_key(batch)
    merge = merge_literature_graph(batch, root=root)
