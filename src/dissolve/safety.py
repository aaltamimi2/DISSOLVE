...
def _validate_safety_plan(
    calls: list[dict[str, Any]], query: str, context: dict[str, Any],
) -> tuple[str, ...]:
    """Preserve an inherited candidate comparison through the safety handoff."""
    call = calls[0] if calls else {}
    args = call.get("args") or {}
    if isinstance(args, str):
        args = json.loads(args)
    candidate_rows, _ = candidate_evidence(
        context,
    )
    ...
    if explicit_candidates is None and any(
        args.get(key) not in (None, [], {})
        for key in explicit_candidate_keys
    ):
        explicit_candidates = []
    _, inherited = resolve_candidate_argument(
        explicit_candidates, candidate_rows,
    )
    expected = []
    seen_expected: set[tuple[str, float]] = set()
    if inherited:
        for item in candidate_rows:
            if not item.get("solvent"):
                continue
            temperature = (
            ...
            if temperature is None:
                continue
            key = (
                thermo.resolve_solvent(str(item["solvent"])) or _key(item["solvent"]),
                float(temperature),
            )
            if key not in seen_expected:
                expected.append((item, float(temperature)))
                seen_expected.add(key)
    if call.get("name") == "screen_route_solvent_substitutions":
        declared_atmospheric = context.get("declared_require_atmospheric")
        expected_atmospheric = True if declared_atmospheric is None else bool(declared_atmospheric)
        if bool(args.get("require_atmospheric", True)) != expected_atmospheric:
            return (
                "Preserve the root planner's typed atmospheric declaration: "
                f"require_atmospheric={str(expected_atmospheric).lower()}.",
            )
        # The declaration defaults to the atmospheric tool default.
        # Query vocabulary never overrides the typed operating mode.
        # Negation and paraphrase are resolved by the root planner.
        # Downstream tool arguments remain independently validated.
    if call.get("name") == "screen_green_solvent_candidates" and isinstance(args, dict):
        requested = list(dict.fromkeys(
            identity for _, identity in thermo.find_polymer_mentions(query)
        ))
        supplied = list(dict.fromkeys(
            thermo.resolve_polymer_identity(str(item))
            for item in args.get("feed_polymers") or []
        ))
        if requested and set(filter(None, supplied)) != set(requested):
            return ("Preserve every requested feed-polymer identity in the green screen: "
                    + ", ".join(requested) + ".",)
        ceiling = re.search(r"\b(?:below|under|less than)\b[^.\n]{0,35}?(\d+(?:\.\d+)?)\s*(?:°\s*)?[Cc]", query, re.I)
        floor = re.search(r"\b(?:at least|minimum|>=|≥)\s*(\d+(?:\.\d+)?)\s*(?:points?|percentage points?)", query, re.I)
        if ceiling and (
            args.get("temperature_max_c") != float(ceiling.group(1))
            or args.get("strict_maximum") is not True
        ):
            return (f"Preserve the exclusive {float(ceiling.group(1)):g} C ceiling.",)
        if floor and args.get("minimum_selectivity_points") != float(floor.group(1)):
            return (f"Preserve the {float(floor.group(1)):g}-point selectivity floor.",)
    if call.get("name") == "compare_solvent_safety_at_conditions":
        return _validate_safety_comparison_plan(args)
    if not expected:
        return ()
    ...
    expected_conditions = [
        ...
        for item, temperature in expected
    ]
    rendered_conditions = ", ".join(
        (
            f"{condition['solvent']} at "
            f"{condition['temperature_c']:g} C"
        )
        for condition in expected_conditions
    )
    violation = (
        "Use compare_solvent_safety_at_conditions with exactly the typed "
        f"candidates and setpoints: {rendered_conditions}."
    )
...
def _validate_safety_comparison_plan(
    arguments: dict[str, Any],
) -> tuple[str, ...]:
    """Reject executable comparison shapes that omit canonical field names."""
    candidates = arguments.get("candidates")
    if candidates is None:
        return ()
    if not isinstance(candidates, list) or not candidates:
        return (
            "Use a non-empty candidates list of candidate objects, or omit "
            "candidates to inherit the typed shortlist.",
        )
    invalid_identity = any(
        not isinstance(candidate, dict)
        or not str(candidate.get("solvent_name") or "").strip()
        for candidate in candidates
    )
    if invalid_identity:
        conditions = []
        for candidate in candidates:
            normalized = _normalize_safety_candidate(candidate)
            if not isinstance(normalized, dict):
                continue
            name = str(normalized.get("solvent_name") or "").strip()
            temperature = _number(
                normalized.get("operating_temp_c")
                if normalized.get("operating_temp_c") is not None
                else normalized.get("temperature_c")
            )
            if name and temperature is not None:
                conditions.append(f"{name} at {temperature:g} C")
        suffix = (
            " preserve the supplied conditions exactly: "
            + ", ".join(conditions) + "."
            if conditions else ""
        )
        return (
            "Use solvent_name as the canonical identity key for every candidate;"
            + suffix,
        )
    invalid_temperature = next((
        str(candidate["solvent_name"]).strip()
        for candidate in candidates
        if (
            candidate.get("operating_temp_c") is not None
            and _number(candidate.get("operating_temp_c")) is None
        )
    ), None)
    if invalid_temperature:
        return (
            f"Use a finite operating_temp_c for {invalid_temperature}.",
        )
    temperature_alias = next((
        str(candidate["solvent_name"]).strip()
        for candidate in candidates
        if (
            candidate.get("operating_temp_c") is None
            and candidate.get("temperature_c") is not None
        )
    ), None)
    if temperature_alias:
        return (
            f"Use operating_temp_c as the canonical temperature key for "
            f"{temperature_alias}.",
        )
    return ()
...
SAFETY_PROMPT = """You are DISSOLVE's solvent-safety analyst. For one named
solvent call get_solvent_safety_card exactly once, preserving its process
temperature or null when none is known. For two or more candidates call
compare_solvent_safety_at_conditions exactly once. Omit candidates when typed
prior candidates exist so their exact identities and setpoints are inherited;
passing candidates is an explicit override. For a greener substitution in an
existing route, call screen_route_solvent_substitutions exactly once with every
feed polymer, the ordered selected route steps, and inherited temperature bounds;
preserve strict_maximum and copy typed_session_context.declared_require_atmospheric; default it to true when null. Never add candidates, collapse different
temperatures, or infer missing fields. For green- or safety-constrained solvent
discovery without an existing route, call screen_green_solvent_candidates once
with the complete feed and dissolution target. A request excluding chlorinated
or high-hazard solvents uses exclude_chlorinated=true and minimum_g_score=6;
these are a sourced structural exclusion and Good/Excellent G-score screen, not
proof of low heated-process hazard. The tools merge local properties,
G-score, optional PubChem hazards/toxicity, peroxide metadata, heating flags,
data gaps, provenance, and a terminal artifact. Return no prose; the parent
model explains the compact result. BP margin is operability, not proof of safety."""
