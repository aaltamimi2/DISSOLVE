...
def _create_and_simulate_process(
    ...
    *,
    ...
    try:
        ...
        process.tea.finance_interest = float(
            config.get("finance_interest", _FINANCE_INTEREST_DEFAULT)
        )
        process.tea.finance_years = int(
            float(config.get("finance_years", _FINANCE_YEARS_DEFAULT))
        )
        ...
        process.tea.startup_FOCfrac = float(
            config.get("startup_FOCfrac", _STARTUP_FOCFRAC_DEFAULT)
        )
        process.tea.startup_VOCfrac = float(
            config.get("startup_VOCfrac", _STARTUP_VOCFRAC_DEFAULT)
        )
        ...
        process.tea.construction = float(
            config.get("construction", _CONSTRUCTION_DEFAULT)
        )
        ...
        process.tea.property_insurance = float(
            config.get("property_insurance", _PROPERTY_INSURANCE_DEFAULT)
        )
        process.tea.maintenance = float(
            config.get("maintenance", _MAINTENANCE_DEFAULT)
        )
