...
D=Path('/mnt/r/plastchem-euler/audits/incremental-2-2026-09-17')
...
dump(D/'snapshot.json',{'utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'accepted_count':len(accepted),'numeric_records':len(sealed),'deferred':deferred,'new_or_changed_surface_count':len(pending_surfaces),'new_or_changed_numeric_count':len(pending_numeric),'prior_audits':[str(prior),str(additional_prior)],'scope':'Incremental coverage plus overlapping surface checks needed for new numerical records; frozen release and batch-2 unchanged'})
...
Run: `/home/aaltamimi2/.venvs/cosmo-logp/bin/python /home/aaltamimi2/plastchem-euler/scripts/audit_incremental_2_20260917.py` (creates a new directory and refuses overwrite).
