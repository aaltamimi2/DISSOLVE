...
import requests
D=Path('/mnt/r/plastchem-euler/comptox-catchup-2026-09-17/sources')
BASE='https://comptox.epa.gov/dashboard-api/batchsearch/export/'
session=requests.Session()
def stamp(): return datetime.datetime.now(datetime.timezone.utc).isoformat()
response_file=D/'comptox-export-response.txt'
if not response_file.exists():
    r=session.post(BASE,json=json.loads((D/'comptox-export-request.json').read_text()),timeout=60)
    response_file.write_text(r.text)
    (D/'comptox-export-receipt.json').write_text(json.dumps({'url':BASE,'status':r.status_code,'utc':stamp(),'sha256':hashlib.sha256(r.content).hexdigest()},indent=2)+'\n')
    r.raise_for_status()
...
for attempt in range(60):
    ...
    (D/'comptox-export-status.json').write_text(json.dumps({'url':url,'status':r.status_code,'body':r.text,'utc':stamp()},indent=2)+'\n')
    r.raise_for_status()
...
p=D/'comptox-properties.xlsx';p.write_bytes(r.content)
