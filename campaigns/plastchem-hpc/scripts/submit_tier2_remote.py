...
root=Path.home()/'plastchem-euler/tier2-v1';group=sys.argv[1]
...
phase2=[line for line in all_queue.splitlines() if len(line.split('|'))>=5 and line.split('|')[1].startswith('contam-p2-')]
assert not phase2, 'Phase 2 keeps priority; defer tier2 initial release while any Phase 2 task remains'
assert m['concurrency']==64
with (p/'submission.started').open('x') as f:f.write(r['utc']+'\n')
result=subprocess.run(['sbatch','--parsable','--hold','--array='+array,'--chdir='+str(root),'--output='+str(root/'logs/%A_%a.out'),'--error='+str(root/'logs/%A_%a.err'),str(root/'tier2.sbatch')],capture_output=True,text=True)
