...
R=Path(__file__).resolve().parents[1];D=Path('/mnt/r/plastchem-euler/polymer-v1/route-comparison-a6')
...
inputs=json.loads((D/'inputs.json').read_text());ensemble=list(csv.DictReader((D/'pe-ensemble.csv').open()));models=[m for m in json.loads((R/'state/polymer-v1/body/manifest.json').read_text())['molecules'] if m['polymer']=='pe'];polymers=[D/'converted-A'/(m['conformer_id']+'.cosmo') for m in models]
rows=[]
for solute in inputs['solutes']:
 ...
 bacts=[json.loads((D/'activities'/f'B-{solute}-pe-{i:02}.json').read_text()) for i in range(31)]
 ...
 for solvent in inputs['solvents']:
  ...
  a=ref.compute_log10_p_solvent_over_polymer(inputs['solutes'][solute]['A'],solvent['A'],polymers,polymer_name='pe',solvent_key=name,ln_gamma=gamma,energies_hartree=[float(r['A_energy_hartree']) for r in ensemble])
