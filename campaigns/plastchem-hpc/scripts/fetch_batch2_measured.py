...
import requests
ROOT=Path(__file__).resolve().parents[1];B=Path('/mnt/r/plastchem-euler/batch-2');D=B/'reference-sources';S=ROOT/'state/batch-2/pubchem-logkow-expansion.json'
parser=argparse.ArgumentParser();parser.add_argument('--all-cohort',action='store_true');args=parser.parse_args()
if args.all_cohort:S=ROOT/'state/batch-2/pubchem-all590-retrieval.json'
cohort=json.loads((B/'manifest.json').read_text())['cohort'];rows=[];session=requests.Session()
def fetch(url,path):
...
for member in cohort:
 key=member['inchikey'];record=json.loads((B/'freeze/state/campaign-v1/records'/f'{key}.json').read_text());m=record['input']
