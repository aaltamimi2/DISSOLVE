"""One local serial thermodynamic worker, incremental over verified campaign and solvent returns."""
import os
os.environ['OMP_NUM_THREADS']='1';os.environ['OPENBLAS_NUM_THREADS']='1'
import fcntl,gc,hashlib,json,time,resource,subprocess
from pathlib import Path
from thermodynamic_library import registry,ROOT,P
from thermodynamic_prediction import activity,pair,validation,CONFIG,refresh_volumes
DEST=Path('/mnt/r/plastchem-euler/thermodynamics-v1');DEST.mkdir(exist_ok=True)
lock=(P/'worker.lock').open('a');fcntl.flock(lock,fcntl.LOCK_EX|fcntl.LOCK_NB)
...
def input_signature():
 ...
 paths=list((ROOT/'state/campaign-v1/records').glob('*.json'))+list((ROOT/'state/solvent-library-v1/records').glob('*.json'))
 paths += [P/name for name in ['solvent-inventory.json','additional-molar-volumes.json','supplier-molar-volumes.json']]
 stamps=[]
...
while True:
 ...
 try:
  ...
  records=[]
  for f in sorted((ROOT/'state/campaign-v1/records').glob('*.json')):
  ...
  for r in records:
   ...
   else:out={'input_inchikey':key,'perceived_inchikey':r['perceived_inchikey'],'identity_match_basis':r['identity_match_basis'],'name':r['input']['name'],'cpu_model':r['cpu_model'],'solute_surface_sha256':r['surface_sha256'],'config':CONFIG,'activities':{}}
  ...
  summary={'utc':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()),'goal_complete':False,'campaign_converged_snapshot':len(records),'library_ready':len(ready),'library_requested':33,'contaminants_processed':len(ledger),'contaminants_full_partitioning':sum(r['complete'] for r in ledger.values()),'available_nonself_partition_predictions':sum(r['partition_count'] for r in ledger.values()),'failed_activity_calculations':sum(r['failed_activity_count'] for r in ledger.values()),'memory_paused':available()<1000*1024,'result_root':str(DEST)}
  local_write(P/'processing-summary.json',summary);print(json.dumps(summary),flush=True)
  export_signature=hashlib.sha256(json.dumps({'ledger':ledger,'library':lib,'campaign':[(f.stem,hashlib.sha256(f.read_bytes()).hexdigest()) for f in sorted((ROOT/'state/campaign-v1/records').glob('*.json'))]},sort_keys=True).encode()).hexdigest()
  export_state=P/'last-automatic-export.json'
  prior_export=json.loads(export_state.read_text()) if export_state.exists() else {}
  if prior_export.get('signature')!=export_signature:
   import sys
   subprocess.run([sys.executable,str(ROOT/'scripts/export_thermodynamic_table.py')],check=True)
   local_write(export_state,{'signature':export_signature,'utc':summary['utc']})
