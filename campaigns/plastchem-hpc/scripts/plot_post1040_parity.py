"""Cumulative 55-entry parity artifact; preserves all earlier report figures."""
import csv,json,hashlib,datetime
from pathlib import Path
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt
D=Path('/mnt/r/plastchem-euler/post1040-octanol-2026-09-15');P=Path('/mnt/r/plastchem-euler/measured-expansion-2026-09-15')
rows=list(csv.DictReader((D/'cumulative-parity.csv').open()));old=rows[:53];new=rows[53:]
s=json.loads((D/'experimental-statistics.json').read_text());assert len(rows)==s['n']==55
...
for group,label,color in [(old,'Previous 53 entries','#4676A9'),(new,'2 additional OPERA matches','#D27624')]:
...
print('Wrote 55-entry CSV and 300 dpi PNG')
