...
D=Path('/mnt/r/plastchem-euler/audits/incremental-6-2026-09-17')
...
fifth_prior=Path('/mnt/r/plastchem-euler/audits/incremental-4-2026-09-17')
prior_surfaces.update({r['inchikey']:r.get('surface_sha256') for r in json.loads((fifth_prior/'completed-surface-audit.json').read_text())['rows'] if r['status']=='passed'})
prior_numeric.update(json.loads((fifth_prior/'sealed-thermodynamics/processing-ledger.json').read_text()))
sixth_prior=Path('/mnt/r/plastchem-euler/audits/incremental-5-2026-09-17')
prior_surfaces.update({r['inchikey']:r.get('surface_sha256') for r in json.loads((sixth_prior/'completed-surface-audit.json').read_text())['rows'] if r['status']=='passed'})
prior_numeric.update(json.loads((sixth_prior/'sealed-thermodynamics/processing-ledger.json').read_text()))
...
dump(D/'snapshot.json',{'utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'accepted_count':len(accepted),'numeric_records':len(sealed),'deferred':deferred,'new_or_changed_surface_count':len(pending_surfaces),'new_or_changed_numeric_count':len(pending_numeric),'prior_audits':[str(prior),str(additional_prior),str(third_prior),str(fourth_prior),str(fifth_prior),str(sixth_prior)],'scope':'Incremental coverage plus overlapping surface checks needed for new numerical records; frozen release and batch-2 unchanged'})
...
Run: `/home/aaltamimi2/.venvs/cosmo-logp/bin/python /home/aaltamimi2/plastchem-euler/scripts/audit_incremental_6_20260917.py` (creates a new directory and refuses overwrite).
