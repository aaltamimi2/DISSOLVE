"""OPERA-only independent accuracy comparison; preserves all earlier packages."""
import csv,json,hashlib,math,datetime
from pathlib import Path
import numpy as np
R=Path('/mnt/r/plastchem-euler');D=R/'octanol-catchup-2026-09-17';E=R/'measured-expansion-2026-09-17'
...
for p in [*R.glob('post*-octanol-2026-09-15/numerical-audit.json'),D/'numerical-audit.json']:
...
for obs in csv.DictReader((E/'experimental-reference-candidates.csv').open()):
 ...
 rows.append({'input_inchikey':k,'name':obs['name'],'measured_logKow':x,'predicted_logKow':y,'residual':y-x,'source_url':obs['source_url'],'citation':obs['raw_reference_string'],'retrieved_utc':obs['retrieved_utc'],'result_path':str(p),'result_sha256':sha})
...
ax.plot([lo,hi],[lo,hi],color='black',linestyle='--');ax.set(xlim=(lo,hi),ylim=(lo,hi),xlabel='Experimental logKow (OPERA curated)',ylabel='Predicted logKow (dry octanol)',title='Octanol–water validation');ax.set_aspect('equal')
