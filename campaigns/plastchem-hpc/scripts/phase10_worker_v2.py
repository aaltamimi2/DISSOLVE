...
def main(planpath, index):
    ...
    pins_path = D / plan['worker_pins_file']
    ...
    signature = dict(plan_sha256=sha(planpath), worker_pins_sha256=sha(pins_path),
    ...
    for u in units:
        assert u['id'] in [member['id'] for member in u['control_original_batch']]
        for member in u['control_original_batch']:
            cache.get(D / member['surface'], member['surface_sha256']); guard()
    ...
    for s in controls + manifest['solvents']:
        ...
        def activity():
            ...
            if s in controls:
                batches = {}
                for u in units:
                    members = u['control_original_batch']
                    ids = tuple(member['id'] for member in members)
                    if ids not in batches:
                        ps = [cache.get(D / member['surface'], member['surface_sha256']) for member in members]
                        batches[ids] = list(map(float, infinite_dilution(ps, phase)))
                    values.append(batches[ids][ids.index(u['id'])])
                return dict(values=values, solvent=s['name'], phase_sha256=s['surface_sha256'],
                    ...
                    comparison_basis='Identical original A-9 subbatch membership and order')
