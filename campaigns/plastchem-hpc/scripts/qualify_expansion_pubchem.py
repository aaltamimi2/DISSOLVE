...
ROOT=Path(__file__).resolve().parents[1];B=Path('/mnt/r/plastchem-euler/measured-expansion-2026-09-15');D=B/'pubchem-measured-validation';D.mkdir(exist_ok=True)
retrieval=json.loads((ROOT/'state/measured-expansion/pubchem-all590-retrieval.json').read_text());observations=[];rejected=[]
def infos(obj):
