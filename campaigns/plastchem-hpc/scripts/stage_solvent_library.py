...
ROOT=Path(__file__).resolve().parents[1];P=ROOT/'state/solvent-library-v1';group=sys.argv[1]
assert group in ['solvent_library']
...
files=[(ROOT/'scripts/solvent_library_runner.py','solvent_library_runner.py'),(ROOT/f'scripts/campaign-{group}.sbatch',f'campaign-{group}.sbatch'),(ROOT/'scripts/submit_solvent_library_remote.py','submit_solvent_library_remote.py'),(P/group/'manifest.json',f'{group}/manifest.json'),(P/group/'submission-indices.json',f'{group}/submission-indices.json')]
for i in indices:
