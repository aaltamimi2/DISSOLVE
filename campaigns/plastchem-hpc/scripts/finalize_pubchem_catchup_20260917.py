...
R=Path(__file__).resolve().parents[1];D=Path('/mnt/r/plastchem-euler/pubchem-catchup-2026-09-17');pid=666349
while Path(f'/proc/{pid}').exists():
 assert 'fetch_pubchem_catchup_20260917.py' in Path(f'/proc/{pid}/cmdline').read_text();time.sleep(30)
r=json.loads((D/'retrieval.json').read_text());assert len(r['rows'])==r['cohort'],'Retrieval terminated before cohort disposition'
subprocess.run(['python',str(R/'scripts/qualify_pubchem_catchup_20260917.py')],check=True)
...
for name in ['fetch_pubchem_catchup_20260917.py','qualify_pubchem_catchup_20260917.py','finalize_pubchem_catchup_20260917.py']:(D/name).write_bytes((R/'scripts'/name).read_bytes())
