...
D=Path('/mnt/r/plastchem-euler/measured-expansion-2026-09-15');S=D/'sources';cohort=json.loads((D/'cohort.json').read_text());receipt=json.loads((S/'OPERA_Data.retrieval.json').read_text());p=S/'LogP_QR.sdf';raw=p.read_bytes();digest=hashlib.sha256(raw).hexdigest();rows=[]
...
for split,name in [('1','LogP_training_observed.csv'),('0','LogP_test_observed.csv')]:write(S/name,[r for r in rows if r.get('Tr_1_Tst_0')==split],fields)
