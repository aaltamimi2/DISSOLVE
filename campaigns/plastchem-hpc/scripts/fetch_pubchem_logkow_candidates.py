...
import requests
ROOT=Path(__file__).resolve().parents[1];B=Path('/mnt/r/plastchem-euler/progress-2026-09-14');D=B/'reference-sources';S=ROOT/'state/progress-2026-09-14/pubchem-logkow-expansion.json'
parser=argparse.ArgumentParser();parser.add_argument('--all-cohort',action='store_true');args=parser.parse_args()
if args.all_cohort:S=ROOT/'state/progress-2026-09-14/pubchem-all590-retrieval.json'
cohort=json.loads((B/'rehearsal/freeze/manifest.json').read_text())['cohort'];rows=[];session=requests.Session()
def fetch(url,path):
...
for member in cohort:
 key=member['inchikey'];record=json.loads((B/'rehearsal/freeze/state/campaign-v1/records'/f'{key}.json').read_text());m=record['input']
 ...
 rows.append(row);S.write_text(json.dumps({'updated_utc':dt.datetime.now(dt.timezone.utc).isoformat(),'cohort':590,'rows':rows,'interpretation':'Raw candidate retrieval only. Modelled/estimated values must not enter experimental-reference CSV.'},indent=2)+'\n')
