...
ROOT=Path(__file__).resolve().parents[1];R=Path('/mnt/r/plastchem-euler');D=R/'octanol-post3883-2026-09-17'
assert not D.exists()
prior={r['input_inchikey'] for r in csv.DictReader((R/'measured-expansion-2026-09-15/all-predictions.csv').open())}
for p in [*R.glob('post*-octanol-2026-09-15/manifest.json'),R/'octanol-catchup-2026-09-17/manifest.json',R/'octanol-followup-2026-09-17/manifest.json']:
 prior.update(m['inchikey'] for m in json.loads(p.read_text())['cohort'])
assert len(prior)==3883
raw=(ROOT/'state/thermodynamics-v1/processing-ledger.json').read_bytes();ledger=json.loads(raw)
measured={r['input_inchikey'] for r in csv.DictReader((R/'combined-validation-references-2026-09-17-pubchem/experimental-reference-candidates.csv').open())}
cohort=[]
...
(ROOT/'state/octanol-post3883-prepared-20260917.json').write_text(json.dumps({'directory':str(D),'manifest_sha256':hashlib.sha256((D/'manifest.json').read_bytes()).hexdigest(),**{k:v for k,v in m.items() if k!='cohort'}},indent=2)+'\n')
