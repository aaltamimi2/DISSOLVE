...
from phase10_tail_common import process,checked_process,sha,save,partition_ownership,keyname
...
def main(chunk):
    ...
    assert gate['status']=='passed' and gate['exact_batch_control_comparisons']==80 and gate['max_control_difference']<=1e-9
    pins=json.loads((D/'tail-code-pins.json').read_text())
    for name,digest in pins.items():assert sha(D/name)==digest,name
    for name,pin in gate['file_pins'].items():assert sha(D/name)==pin,name
    plan=json.loads((D/'production-plan.json').read_text());units=plan['chunks'][chunk]
    manifest=json.loads((D/'manifest.json').read_text())
    out=D/plan['output']/f'{chunk:04d}'
    starts=list(out.glob('started-*.json'));assert len(starts)==1,'Reconcile prior executions before splitting'
    ...
    for p in Path('/proc').iterdir():
        ...
        if record['cwd']==str(D) and record['command'].endswith(f'python -u phase10_worker_v2.py production-plan.json {chunk}'):
    ...
    try:
        ...
        assignment=dict(ownership,utc=utc(),chunk=chunk,original_process=original,signature=started['signature'],
                        ...
                        gate_sha256=sha(D/'calibration-clearance.json'))
        ...
        while True:
            ...
            receipt_path=D/f'production-retry-tail{chunk:04d}-submission.json'
            pending=D/f'production-retry-tail{chunk:04d}-unconfirmed.json'
        ...
        with (root/'launch.lock').open('a') as final_lock:
            ...
            if state.get('status') not in ['helpers_terminal_resume_ready','aborted_before_submission']:
                if (D/f'production-retry-tail{chunk:04d}-submission.json').exists() or (D/f'production-retry-tail{chunk:04d}-unconfirmed.json').exists():
