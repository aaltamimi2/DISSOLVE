...
while True:
    try:
        response=run('ssh',['euler',f'python3 {REMOTE}/collect_tier2_remote.py {since}'],capture_output=True,text=True)
        ...
        if fresh:
            ...
            if fetch:
                result=run('scp',['-rq',*[f'euler:plastchem-euler/tier2-v1/returns/{key}' for key in fetch],str(DEST)],capture_output=True,text=True)
        ...
        subprocess.run(['python3',str(ROOT/'scripts/summarize_tier2.py')],check=True,stdout=subprocess.DEVNULL)
