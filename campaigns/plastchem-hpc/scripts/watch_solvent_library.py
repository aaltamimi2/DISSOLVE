...
ROOT=Path(__file__).resolve().parents[1];P=ROOT/'state/solvent-library-v1';REMOTE='~/plastchem-euler/solvent-library-v1'
DEST=Path('/mnt/r/plastchem-euler/solvent-library')
...
while True:
    try:
        response=run('ssh',['euler',f'python3 {REMOTE}/collect_solvent_library_remote.py {since}'],capture_output=True,text=True)
        if response.returncode:raise RuntimeError(response.stderr[:500])
        snap=json.loads(response.stdout);write(P/'latest-snapshot.json',snap)
        remote_records.update(snap['changed_records']);write(remote_records_path,remote_records);since=snap['epoch']
        ...
        for task,mol in known.items():
            ...
            r=dict(r or {'input':mol,'inchikey':key,'scope':'thermodynamic_solvent_library','group':mol['group'],'cpu_model':None,'node':acct[8] if acct else None,'dft_ran':False})
        ...
        if fresh:
            ...
            if fetch:
                result=run('scp',['-rq',*[f'euler:plastchem-euler/solvent-library-v1/returns/{key}' for key in fetch],str(DEST)],capture_output=True,text=True)
        ...
        subprocess.run(['python3',str(ROOT/'scripts/summarize_solvent_library.py')],check=True,stdout=subprocess.DEVNULL)
        ...
        if len(snap['groups'])==1 and not snap['squeue'].strip() and all(summary['counts'][k]==0 for k in ['running','not_yet_run','awaiting_verification']):
            print('SOLVENT_LIBRARY_TERMINAL_AND_VERIFIED',flush=True);break
    ...
    time.sleep(max(600,transport.get('retry_after_epoch',0)-time.time()))
