...
D=R/'cumulative-figures-2026-09-17';D.mkdir(exist_ok=False)
...
for s,col in zip(cats,['#46906b','#b95c4b','#dbb45d','#b5bbc3']):
 vals=np.array([r[s] for r in progress]);ax.barh([r['chunk'].replace('main_chunk_','Main ').replace('tail_gt80','Tail >80').replace('pilot_reuse','Pilot reuse') for r in progress],vals,left=left,color=col,label=s.replace('_',' '));left+=vals
...
for p in [*R.glob('post*-octanol-2026-09-15/numerical-audit.json'),R/'octanol-catchup-2026-09-17/numerical-audit.json']:
