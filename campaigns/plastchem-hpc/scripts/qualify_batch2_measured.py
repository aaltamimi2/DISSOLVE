...
ROOT=Path(__file__).resolve().parents[1];B=Path('/mnt/r/plastchem-euler/batch-2');D=B/'pubchem-measured-validation';D.mkdir(exist_ok=True)
retrieval=json.loads((ROOT/'state/batch-2/pubchem-all590-retrieval.json').read_text());observations=[];rejected=[]
def infos(obj):
...
cohort=json.loads((B/'manifest.json').read_text())['cohort']
with (D/'per-molecule-best-measured.csv').open('w',newline='') as f:
 ...
 for member in cohort:
  ...
  if key in best:w.writerow({**best[key],'reference_status':'qualified_point_value'})
   ...
   record=json.loads((B/'freeze/state/campaign-v1/records'/f'{key}.json').read_text())
