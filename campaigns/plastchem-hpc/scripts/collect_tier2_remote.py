...
root=Path.home()/'plastchem-euler/tier2-v1';since=float(sys.argv[1]) if len(sys.argv)>1 else 0
now=time.time();groups={}
for group in ['tier2'] + sorted(p.name for p in root.glob('main_chunk_[0-9][0-9][0-9]')):
...
names=','.join(['contam-tier2-chno500700-v1']+[json.loads((root/g/'manifest.json').read_text())['name'] for g in groups if g.startswith('main_chunk_')])
