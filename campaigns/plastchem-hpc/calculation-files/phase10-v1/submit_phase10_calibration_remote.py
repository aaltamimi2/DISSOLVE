...
NAME = 'contam-phase10-calibration-v1'
...
def save():
    p = D / 'calibration-submission.json'
...
for name, digest in json.loads((D / 'staging-pins.json').read_text()).items():
...
receipt.update(squeue_reconciliation=queue, sacct_reconciliation=account,
               plan_sha256=sha(D / 'calibration-plan.json'))
found = {s.split('|')[0].split('_')[0].split('.')[0] for s in (queue + '\n' + account).splitlines()
         if re.match(r'^\d', s)}
if found:
    assert len(found) == 1
    job = next(iter(found))
    prior = D / 'calibration-submission.json'
    if prior.exists():
    ...
    assert not (D / 'calibration-submission-unconfirmed.json').exists(), 'Unconfirmed attempt requires reconciliation'
    assert not (D / 'calibration-submission.json').exists(), 'Receipt exists but scheduler history absent'
    command = ['sbatch', '--parsable', '--hold', '--job-name=' + NAME, '--partition=research',
        ...
        '--chdir=' + str(D), '--output=' + str(D / 'logs/calibration-%A_%a.out'),
        '--error=' + str(D / 'logs/calibration-%A_%a.err'),
        '--wrap=export OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 PYTHONPATH=../phase8-v1; '
        '../phase8-v1/venv/bin/python -u phase10_worker.py calibration-plan.json "$SLURM_ARRAY_TASK_ID"']
    receipt['command'] = command
    (D / 'calibration-submission-unconfirmed.json').write_text(json.dumps(receipt, indent=2) + '\n')
