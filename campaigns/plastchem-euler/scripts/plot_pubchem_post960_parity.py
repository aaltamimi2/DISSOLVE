"""Cumulative 57-entry parity artifact; preserves all earlier report figures."""
import csv,json,hashlib,datetime
from pathlib import Path
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt
D=Path('/mnt/r/plastchem-euler/pubchem-post960-2026-09-15');P=Path('/mnt/r/plastchem-euler/measured-expansion-2026-09-15')
rows=list(csv.DictReader((D/'cumulative-parity.csv').open()));old=rows[:55];new=rows[55:]
s=json.loads((D/'experimental-statistics.json').read_text());assert len(rows)==s['n']==57
...
for group,label,color in [(old,'Previous 55 entries','#4676A9'),(new,'2 additional PubChem matches','#D27624')]:
...
print('Wrote 57-entry CSV and 300 dpi PNG')
