...
import numpy as np
R=Path('/mnt/r/plastchem-euler');D=R/'octanol-followup-2026-09-17';E=R/'combined-validation-references-2026-09-17-pubchem-final571'
newaudit=json.loads((D/'numerical-audit.json').read_text());assert newaudit['failed']==0
prior_choices={r['input_inchikey']:r for r in csv.DictReader((R/'validation-comptox-final1631-2026-09-17/parity.csv').open())}
lookup={}
...
accepted={p.stem for p in Path('/home/aaltamimi2/plastchem-euler/state/campaign-v1/records').glob('*.json') if json.loads(p.read_text())['status']=='converged'}
assert set(lookup)==accepted and len(accepted)==5803,'Audited octanol union must cover the full accepted cohort'
rows=[];unmatched=[]
...
for r in rows:
 if r['input_inchikey'] in prior_choices:
  prior=prior_choices[r['input_inchikey']];assert r['measured_logKow']==float(prior['measured_logKow']) and r['predicted_logKow']==float(prior['predicted_logKow'])
x=np.array([r['measured_logKow'] for r in rows]);y=np.array([r['predicted_logKow'] for r in rows]);error=y-x
slope,intercept=np.polyfit(x,y,1)
s={'utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'scope':'Previous n622 choices retained; additional qualified PubChem, OPERA and CompTox values, matched to audited octanol results','n':len(rows),'MAE':float(abs(error).mean()),'RMSE':float(np.sqrt((error**2).mean())),'bias':float(error.mean()),'slope':float(slope),'intercept':float(intercept),'unmatched_reference_count':len(unmatched),'outliers':sorted(rows,key=lambda r:abs(r['residual']),reverse=True)[:10],'no_recalibration':True}
O=R/'validation-final5803-2026-09-17';O.mkdir(exist_ok=False)
