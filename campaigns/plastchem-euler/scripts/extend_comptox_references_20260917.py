...
R=Path('/mnt/r/plastchem-euler');D=R/'combined-validation-references-2026-09-17-comptox-later'
p=R/'combined-validation-references-2026-09-17-pubchem/experimental-reference-candidates.csv'
q=R/'comptox-later-2026-09-17/comptox-experimental-reference-candidates.csv'
old=list(csv.DictReader(p.open()));observations=list(csv.DictReader(q.open()));selected={r['input_inchikey']:r for r in old}
assert len(selected)==len(old)==410
...
s={'utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'retained':len(old),'added':len(rows)-len(old),'selected_entries':len(rows),'connectivity_blocks':len({r['input_inchikey'].split('-')[0] for r in rows}),'source_sha256':{str(f):hashlib.sha256(f.read_bytes()).hexdigest() for f in [p,q]},'selection':'Preserve all 410 prior selections. For newly covered keys only, rank qualified experimental point observations by selection_priority, source name, citation. No predicted value is consulted.'}
(D/'summary.json').write_text(json.dumps(s,indent=2)+'\n')
(D/'REPORT.md').write_text('# Extended measured-reference coverage\n\n'+json.dumps(s,indent=2)+'\n\nCoverage is not paired accuracy n. All observations retain source, retrieval time and citation. CompTox experimental classifications are preserved; primary papers and measurement conditions have not all been independently inspected. Compilation sources can overlap and are not independent replicates. No recalibration. Xylene identity and didecyl-phthalate reference discrepancy remain owner questions.\n')
