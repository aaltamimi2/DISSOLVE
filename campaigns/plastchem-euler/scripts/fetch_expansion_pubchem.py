...
import requests
ROOT=Path(__file__).resolve().parents[1];B=Path('/mnt/r/plastchem-euler/measured-expansion-2026-09-15');D=B/'reference-sources';S=ROOT/'state/measured-expansion/pubchem-logkow-expansion.json'
parser=argparse.ArgumentParser();parser.add_argument('--all-cohort',action='store_true');args=parser.parse_args()
if args.all_cohort:S=ROOT/'state/measured-expansion/pubchem-all590-retrieval.json'
