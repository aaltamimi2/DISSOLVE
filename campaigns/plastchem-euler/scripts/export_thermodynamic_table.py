...
ROOT=Path(__file__).resolve().parents[1];P=ROOT/'state/thermodynamics-v1';DEST=Path('/mnt/r/plastchem-euler/thermodynamics-v1')
...
eligible=json.loads((record_root/'state/campaign-v1/eligible.json').read_text());library=json.loads((P/'library-registry.json').read_text());solvents=[s for s in library['solvents'] if s['solvent_key']!='water'];ledger=json.loads((P/'processing-ledger.json').read_text()) if (P/'processing-ledger.json').exists() else {}
phase_review=json.loads((P/'solvent-phase-review.json').read_text());assert phase_review['temperature_K']==298.15
phase_notes={r['solvent']:r for r in phase_review['entries']}
fields=['phase_basis','solvent_phase_note','solvent_phase_source','input_inchikey','perceived_inchikey','identity_match_basis','name','orca_status','solvent','reference','status','reason','temperature_K','parameterization','log10_K_mole_fraction','log10_K_concentration','concentration_basis_status','volume_correction_log10','volume_source','observed_dilution_shift_sum_log10','solute_surface_sha256','solvent_surface_sha256','reference_surface_sha256']
...
with output.open('w',newline='') as f:
 ...
 for mol in eligible:
  key=mol['inchikey'];record=record_root/'state/campaign-v1/records'/(key+'.json');r=json.loads(record.read_text()) if record.exists() else {};data={}
  if key in ledger:
  ...
  for solvent in solvents:
   name=solvent['solvent_key'];row={'input_inchikey':key,'perceived_inchikey':r.get('perceived_inchikey'),'identity_match_basis':r.get('identity_match_basis'),'name':mol['name'],'orca_status':r.get('status','not_yet_run'),'solvent':name,'reference':'water','temperature_K':298.15,'parameterization':'openCOSMORS24a','solute_surface_sha256':r.get('surface_sha256')}
...
assert len(eligible)==5824 and len(solvents)==32 and n==5824*32
h=hashlib.sha256()
with output.open('rb') as f:
 for block in iter(lambda:f.read(1024*1024),b''):h.update(block)
summary={'utc':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()),'status':'incomplete','eligible_contaminants':5824,'isotope_exclusions':9,'requested_nonself_solvent_pairs_per_contaminant':32,'table_rows':n,'row_status_counts':dict(counts),'phase_review_sha256':hashlib.sha256((P/'solvent-phase-review.json').read_bytes()).hexdigest(),'csv_path':str(output),'csv_bytes':output.stat().st_size,'csv_sha256':h.hexdigest(),'notes':'Missing values remain empty. This export is not evidence of complete thermodynamic coverage.'}
