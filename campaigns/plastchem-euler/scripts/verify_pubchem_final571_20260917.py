...
D=Path('/mnt/r/plastchem-euler/pubchem-final571-2026-09-17');pid=1215557
...
assert len(rows)==s['best_point_value_molecules']
...
r={'utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'files_verified':len(lines),'manifest_sha256':hashlib.sha256((D/'artifacts.sha256').read_bytes()).hexdigest(),'qualified_molecules':len(rows),'qualified_observations':s['qualified_observations'],'scope':'Package integrity and qualification-rule checks; not independent primary-paper verification'}
Path('state/pubchem-final571-verified-20260917.json').write_text(json.dumps(r,indent=2)+'\n');print(json.dumps(r),flush=True)
