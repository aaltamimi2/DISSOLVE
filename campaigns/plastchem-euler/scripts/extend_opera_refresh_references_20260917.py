...
R=Path('/mnt/r/plastchem-euler');D=R/'combined-validation-references-2026-09-17-opera-refresh'
p=R/'combined-validation-references-2026-09-17-comptox-later/experimental-reference-candidates.csv'
q=R/'measured-expansion-2026-09-17-refresh4353/opera-experimental-reference-candidates.csv'
old=list(csv.DictReader(p.open()));observations=list(csv.DictReader(q.open()));selected={r['input_inchikey']:r for r in old}
assert len(selected)==len(old)==447
...
s={'utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'retained':len(old),'added':len(rows)-len(old),'selected_entries':len(rows),'connectivity_blocks':len({r['input_inchikey'].split('-')[0] for r in rows}),'source_sha256':{str(f):hashlib.sha256(f.read_bytes()).hexdigest() for f in [p,q]},'selection':'Preserve all 447 prior selections. For newly covered keys only, rank qualified experimental point observations by selection_priority, source name, citation. No predicted value is consulted.'}
