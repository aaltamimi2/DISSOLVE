...
ROOT=Path(__file__).resolve().parents[1];D=Path('/mnt/r/plastchem-euler/octanol-post4261-2026-09-17')
...
assert a['denominator']==a['passed']==s['denominator']==92 and a['failed']==0
assert s['octanol_predicted']+s['octanol_failed']==92
assert json.loads((ROOT/'state/octanol-post4261-worker-handoff-20260917.json').read_text())['batch_exit_code']==0
assert m['matched_selected_reference_count']==0
(D/'REPORT.md').write_text(f"# Audited octanol catch-up\n\nThis disjoint cohort contains 92 accepted contaminants. Predictions: {s['octanol_predicted']}/92; explicitly unavailable: {s['octanol_failed']}/92. All 92 outcomes passed the provenance, identity, surface-hash, dilution and partition-algebra audit. Numerical verification does not establish experimental accuracy or full solvent-panel coverage.\n\nEarlier audited octanol predictions: 4,261; cumulative available predictions: {4261+s['octanol_predicted']}. This cohort adds no matched measured references, so the n=447 experimental comparison in `/mnt/r/plastchem-euler/octanol-post4060-2026-09-17/cumulative-validation/` remains unchanged. No empirical correction or changed reference selection was applied.\n\nThe released 648 and fixed batch-2 cumulative 944 packages remain unchanged. Xylene identity and the didecyl-phthalate reference discrepancy remain owner questions. The panel-solvent library is still incomplete.\n\nFrozen manifest and source copies support reproduction. Run `process_octanol_catchup_20260917.py` with PLASTCHEM_OCTANOL_FILL_ROOT set to this directory, then the cohort auditor; processing is serial and resumable.\n")
code=D/'code';code.mkdir(exist_ok=True)
for name in ['process_octanol_catchup_20260917.py','prepare_octanol_post4261_20260917.py','audit_octanol_post4261_20260917.py',Path(__file__).name]:(code/name).write_bytes((ROOT/'scripts'/name).read_bytes())
...
r={'utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'files_verified':len(files),'manifest_sha256':hashlib.sha256((D/'artifacts.sha256').read_bytes()).hexdigest(),'audit_passed':92,'audit_failed':0,'predicted':s['octanol_predicted'],'cumulative_audited_predictions':4261+s['octanol_predicted']}
(ROOT/'state/octanol-post4261-sealed-20260917.json').write_text(json.dumps(r,indent=2)+'\n');print(json.dumps(r))
