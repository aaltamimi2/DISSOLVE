"""One fixed-manifest Phase 1 molecule, serial frozen ORCA recipe, durable failure data."""
import hashlib,json,os,re,resource,signal,socket,subprocess,time
...
try:
 ...
 prep=json.loads((ROOT/'inputs'/key/'preparation.json').read_text());result['preparation']=prep
 if prep['status']!='prepared':fail('preparation_failed',prep.get('error','Local embedding/MMFF failure'))
 raw=(ROOT/'inputs'/key/'input.xyz').read_bytes()
 ...
 for stage in ['opt','cosmo']:
  ...
  begun=time.monotonic();deadline=11.9*3600-(begun-start)
  if deadline<=0:fail('walltime_censored','Per-job diagnostic deadline exhausted')
  with (work/f'{stage}.out').open('w') as out:
   ...
   try:code=proc.wait(timeout=deadline)
    ...
    fail('walltime_censored','Reached 11.9h elapsed guard; time is right-censored')
