...
ROOT=Path(__file__).resolve().parents[1];D=Path('/mnt/r/plastchem-euler/octanol-post4172-2026-09-17')
...
r={'utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'files_verified':len(files),'manifest_sha256':hashlib.sha256((D/'artifacts.sha256').read_bytes()).hexdigest(),'audit_passed':62,'audit_failed':0,'predicted':s['octanol_predicted'],'cumulative_audited_predictions':4172+s['octanol_predicted']}
(ROOT/'state/octanol-post4172-sealed-20260917.json').write_text(json.dumps(r,indent=2)+'\n');print(json.dumps(r))
