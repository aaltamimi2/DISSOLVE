...
ROOT=Path(__file__).resolve().parents[1];R=Path('/mnt/r/plastchem-euler');D=R/'octanol-post4244-2026-09-17'
assert not D.exists()
...
(ROOT/'state/octanol-post4244-prepared-20260917.json').write_text(json.dumps({'directory':str(D),'manifest_sha256':hashlib.sha256((D/'manifest.json').read_bytes()).hexdigest(),**{k:v for k,v in m.items() if k!='cohort'}},indent=2)+'\n')
