...
live=json.loads((R/'state/tier2-v1/thermodynamics/processing-ledger.json').read_text())
...
accepted={}
for p in (R/'state/tier2-v1/records').glob('*.json'):
 record=json.loads(p.read_text())
 if record.get('status')=='converged': accepted[record['inchikey']]=record
assert len(accepted)==27, 'Accepted cohort changed: review required'
...
for k,v in selected.items():
 ...
 record=json.loads(raw);assert record.get('tier')=='CHNO_500_700' and record.get('tier_denominator')==270;assert set(record['activities'])==ready and record['config']==CONFIG
 ...
 source=R/'state/tier2-v1/records'/f'{k}.json';target=D/'freeze/state/tier2-v1/records'/source.name;target.parent.mkdir(parents=True,exist_ok=True);shutil.copyfile(source,target)
...
save(D/'snapshot.json',{'utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'selected':len(selected),'sealed':len(sealed),'changed_deferred':changed,'scope':'All 27 accepted molecules at 32 resolved-solvent attempts; explicit activity failures retained','volume_source_checks':'Loaded and verified source hashes using refresh_volumes; each concentration result bound to documented solvent volume'})
env=dict(os.environ,PLASTCHEM_PROGRESS_ROOT=str(D));subprocess.run([sys.executable,str(R/'scripts/audit_tier2_thermodynamic_records.py'),'--frozen'],env=env,check=True)
...
for f in ['audit_tier2_first27_panel.py','audit_tier2_thermodynamic_records.py','audit_panel_failure_semantics.py']:shutil.copyfile(R/'scripts'/f,D/f)
manifest=''.join(f'{sha(p)}  {p.relative_to(D)}\n' for p in sorted(D.rglob('*')) if p.is_file());(D/'SHA256SUMS').write_text(manifest)
save(R/'state/tier2-first27-panel-audit.json',{'utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'root':str(D),'passed':audit['passed'],'failed':audit['failed'],'manifest_sha256':sha(D/'SHA256SUMS'),'scope':'First 27 accepted tier-2 contaminants against 32 resolved references; full tier denominator 270; unresolved xylene unavailable','file_count':len(manifest.splitlines())})
