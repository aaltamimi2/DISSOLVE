"""Cumulative 64-entry parity artifact; preserves all earlier report figures."""
import csv,json,hashlib,datetime
from pathlib import Path
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt
D=Path('/mnt/r/plastchem-euler/post1160-octanol-2026-09-15/opera-extension');P=Path('/mnt/r/plastchem-euler/measured-expansion-2026-09-15')
rows=list(csv.DictReader((D/'cumulative-parity.csv').open()));old=rows[:63];new=rows[63:]
s=json.loads((D/'experimental-statistics.json').read_text());assert len(rows)==s['n']==64
...
for group,label,color in [(old,'Previous 63 entries','#4676A9'),(new,'1 additional OPERA match','#D27624')]:
...
print('Wrote 64-entry CSV and 300 dpi PNG')
