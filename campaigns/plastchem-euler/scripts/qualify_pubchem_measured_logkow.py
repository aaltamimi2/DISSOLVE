...
ROOT=Path(__file__).resolve().parents[1];B=Path('/mnt/r/plastchem-euler/progress-2026-09-14');D=B/'pubchem-measured-validation';D.mkdir(exist_ok=True)
retrieval=json.loads((ROOT/'state/progress-2026-09-14/pubchem-all590-retrieval.json').read_text());observations=[];rejected=[]
def infos(obj):
...
for item in retrieval['rows']:
 ...
 for info in infos(doc):
  ...
  for text in texts:
   ...
   elif re.search(r'GEMS|KOWWIN|EPI.?Suite|estimat|predict|calculat',' | '.join(citations),re.I):row['qualification']='excluded_model_or_unverified_computation_citation'
...
with (D/'per-molecule-best-measured.csv').open('w',newline='') as f:
 ...
 for member in cohort:
  ...
  if key in best:w.writerow({**best[key],'reference_status':'qualified_point_value'})
   ...
   record=json.loads((B/'rehearsal/freeze/state/campaign-v1/records'/f'{key}.json').read_text())
