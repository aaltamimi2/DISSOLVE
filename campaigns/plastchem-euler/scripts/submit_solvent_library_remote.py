...
root=Path.home()/'plastchem-euler/solvent-library-v1';group=sys.argv[1]
...
assert m['dependency_array_ids']==['54802','54733']
r['dependency']='afterany:'+':'.join(m['dependency_array_ids'])
result=subprocess.run(['sbatch','--parsable','--dependency='+r['dependency'],'--array='+array,'--chdir='+str(root),'--output='+str(root/'logs/%A_%a.out'),'--error='+str(root/'logs/%A_%a.err'),str(root/f'campaign-{group}.sbatch')],capture_output=True,text=True)
