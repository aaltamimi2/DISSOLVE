...
R=Path('/home/aaltamimi2/plastchem-euler');D=Path('/mnt/r/plastchem-euler/polymer-v1/route-comparison-a7/evoh');D.mkdir(parents=True,exist_ok=True)
...
models=[m for m in json.loads((R/'state/polymer-v1/body/manifest.json').read_text())['molecules'] if m['polymer']=='evoh'];assert len(models)==11
...
for solute in solutes:
 ...
 for route in ['A','B']:
  ...
  w=weights[route];vp=float(sum(w[i]*ensemble[i][route+'_cavity_cm3_mol'] for i in range(11)))
  if complete:
  ...
  for solvent in solvents:
   ...
   if complete and a['status']=='converged':
    ...
    if route=='B':value['OPT_weight_sensitivity_logP_concentration']=(optgamma-a['ln_gamma'])/math.log(10)+math.log10(sum(weights['B_OPT'][i]*ensemble[i]['B_cavity_cm3_mol'] for i in range(11))/solvent['B_volume'])
