...
D=R/'cumulative-figures-2026-09-17-final-orca';D.mkdir(exist_ok=False)
...
assert Counter(r['status'] for r in dispositions)=={'converged':5803,'failed':21}
assert not running
...
ax.set(xlabel='Structures',title='ORCA dispositions / 5,824');ax.legend(loc='upper center',bbox_to_anchor=(.5,-.10),ncol=4);fig.savefig(D/'campaign-progress.png',dpi=300);plt.close(fig)
